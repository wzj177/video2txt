    ...
    async def _enhance_content_with_ai(
        ...
        **kwargs,
        ...
        try:
            ...
            # 1. 构建扩写+润色的系统提示词
            enhancement_system_prompt = f"""你是一位专业的内容编辑和写作专家，擅长对{content_domain}领域的内容进行扩写和润色。

# 任务目标
对提供的内容卡片进行智能扩写和润色，提升内容的质量、可读性和实用性。

# 扩写原则
1. **保持原有结构**：维持原文的章节结构和逻辑框架
2. **深度扩展**：在现有要点基础上增加更详细的解释、举例和分析
3. **专业性提升**：结合{content_domain}领域的专业知识，增加权威性内容
4. **实用性增强**：添加实际应用建议、操作步骤或实践要点
5. **可读性优化**：改善语言表达，使内容更加流畅自然
...
5. **专业性提升**：结合{content_domain}领域的专业知识，增加权威性内容
