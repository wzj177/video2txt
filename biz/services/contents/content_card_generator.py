...
class ContentCardGenerator:
    ...
    async def _generate_text_only_content(
        ...
        try:
            ...
            content_role = config.get("content_role", "general") or "general"
            if content_role == "auto":
                content_role = "general"

            role_name = get_role_name(content_role, "content_card", "内容专家")
            domain_mapping = {
                "tech": "技术",
                "business": "商业",
                "education": "教育",
            ...
            domain = domain_mapping.get(content_role, "通用")
            ...
            role_key = content_role
            prompts = template_skill_service.render_role_template_parts(
                role_key, "content_card", template_vars
            )

            fallback_system_template = """# 角色设定
...
- 保持专业性和可读性的平衡"""
            ...
            fallback_user_template = """请为以下音频转录内容生成结构化的内容卡片：

{transcript}

请生成一个结构清晰、内容精炼的知识卡片。"""

            system_prompt_template = prompts.get("audio_system_prompt") or fallback_system_template
            user_prompt_template = prompts.get("audio_user_prompt") or fallback_user_template

            selected_skill_key = template_skill_service.get_skill_for_role(
                role_key, "content_card"
            )
            system_prompt = self._format_prompt(system_prompt_template, template_vars)
            ...
            user_prompt = self._format_prompt(user_prompt_template, template_vars)
    ...
    async def generate_content_card(
        ...
        custom_user_prompt: str = None,  # 🧠 新增：支持自定义用户提示词
        **kwargs,
    ) -> Dict[str, Any]:
        """生成内容卡片 - 主入口方法"""
        try:
            ...
            logger.info(
                f"调试信息: enhanced_mapping={enhanced_mapping}, subtitles数量={len(subtitles)}, frames数量={len(frames)}"
            )
            logger.info(f"frame_info键: {list(frame_info.keys())}")
            logger.info(f"frame_info完整内容: {frame_info}")
            logger.info(f"kwargs键: {list(kwargs.keys())}")
            logger.info(
                f"enhanced_transcript_file: {kwargs.get('enhanced_transcript_file', 'NOT_FOUND')}"
            )

            if subtitles and len(subtitles) > 0:
                sample_subtitle = subtitles[0]
                logger.info(f"sample_subtitle: {sample_subtitle}")
                has_frame_field = "frame" in sample_subtitle
                logger.info(f"subtitles包含frame字段: {has_frame_field}")
            else:
                logger.error("subtitles为空或未传递！")
                has_frame_field = False
                logger.info(f"subtitles中是否包含frame字段: {has_frame_field}")

            # 2. 帧数据处理和增强版映射检查
            # 关键修复：即使frames为空，如果有增强版映射也要继续处理
            if not frames and not enhanced_mapping:
                logger.info("🎵 无帧数据且无增强版映射，生成纯文本内容卡片")
                return await self._generate_text_only_content(
                    config, transcript, stream_callback
                )

            # 转换帧数据格式（可能为空列表，但增强版映射不依赖这个）
            frame_list = self._convert_frame_data(frames)
            logger.info(f"处理 {len(frame_list)} 个提取的帧")

            # 当使用增强版映射时，frame_list 主要用于：
            # 1. 提示词中的帧文件名统计展示
            # 2. Few-shot示例生成时的随机选择
            # 3. 调试信息和日志输出
            # 实际的帧-时间段映射完全依赖 subtitles 中的增强数据
            ...
            content_domain = content_role or (
                analysis_result.primary_domain.value if analysis_result else "general"
            )
            logger.info(f"✅ 使用内容角色: {content_domain}")

            # 3. 验证增强版映射：检查是否为摘要模式
            # 如果frame_info明确标记不使用增强版映射，则进入纯文本模式（用于摘要生成）
            if enhanced_mapping is False or (not enhanced_mapping and not subtitles):
                logger.info("检测到摘要生成模式，使用纯文本内容卡片生成")
                return await self._generate_text_only_content(
                    config, transcript, stream_callback
                )

            # 正常的AI内容生成模式：必须有增强版映射
            if not enhanced_mapping or not subtitles:
                error_msg = "内容卡片生成必须使用增强版映射（transcription_format.json），当前缺失"
                logger.error(error_msg)
                raise ValueError(error_msg)

            # 验证subtitles包含帧映射信息
            frame_segments = [s for s in subtitles if s.get("frame")]
            if not frame_segments:
                error_msg = "subtitles中没有找到帧映射信息，无法生成内容卡片"
                logger.error(error_msg)
                raise ValueError(error_msg)
            ...
            frame_names = [frame.get("filename", "") for frame in frame_list]
            template_vars = {
                "role_name": role_name,
                ...
                "cover_frame": cover_frame or "",
                "frame_count": len(frame_list),
                "frame_list": ", ".join([name for name in frame_names if name]),
                "mapping_count": len(matches),
            }

            role_key = content_domain or "general"
            prompts = template_skill_service.render_role_template_parts(
                role_key, "content_card", template_vars
            )
            template_system_prompt = (
                prompts.get("video_system_prompt")
                or prompts.get("system_prompt")
                or ""
            )
            template_user_prompt = (
                prompts.get("video_user_prompt")
                or prompts.get("user_prompt")
                or ""
            )

            if custom_system_prompt:
                system_prompt = self._format_prompt(custom_system_prompt, template_vars)
                logger.info("使用智能分析生成的自定义系统提示词")
            elif template_system_prompt:
                system_prompt = self._format_prompt(template_system_prompt, template_vars)
                logger.info("使用模板中的视频系统提示词")
            else:
                system_prompt = self._build_system_prompt(
                    image_strategy, keyframes_path, content_domain, media_duration
                )

            if not kwargs.get("prompts_composed", False):
                selected_skill_key = template_skill_service.get_skill_for_role(
                    role_key, "content_card"
                )
                system_prompt = template_skill_service.build_system_prompt(
            ...
            if custom_user_prompt:
                user_prompt = self._format_prompt(custom_user_prompt, template_vars)
            elif template_user_prompt:
                user_prompt = self._format_prompt(template_user_prompt, template_vars)
            else:
                # 使用增强版映射生成用户提示词
                user_prompt = self._build_user_prompt_enhanced(
                    transcript, subtitles, keyframes_path, content_domain
                )
            ...
            # 6.5. AI扩写+润色处理 (如果启用)
            # 6. AI 扩写+润色（可选）
            enhancement_prompts = {}  # 🆕 收集AI扩写润色的提示词信息
            ai_enhancement = kwargs.get("ai_enhancement", False)
            if ai_enhancement:
                logger.info("启用AI扩写+润色功能，开始增强内容...")
                if stream_callback:
                    await stream_callback(
                        "ai_generating",
                        {
                ...
                # 🆕 收集AI扩写润色的提示词（先构建提示词信息）
                enhancement_config = self._get_enhancement_config()
                logger.info(f"🎨 AI扩写润色使用的content_domain: {content_domain}")
                enhancement_system_prompt, enhancement_user_prompt = (
                    self._build_enhancement_prompts(content, transcript, content_domain)
                )

                # 记录扩写润色提示词信息
                enhancement_prompts = {
                    "enhancement_system_prompt": enhancement_system_prompt,
                    "enhancement_user_prompt": enhancement_user_prompt,
                    "enhancement_timeout": enhancement_config.get("timeout", 180),
                    "enhancement_max_retries": enhancement_config.get("max_retries", 2),
                }

                content = await self._enhance_content_with_ai(
                    content, transcript, content_domain, stream_callback, **kwargs
                )
            ...
            return {
                ...
                "prompts": {  # 🆕 返回提示词信息
                    ...
                    "matches_count": len(matches) if enhanced_mapping else 0,
                    "ai_enhancement_enabled": kwargs.get(
                        "ai_enhancement", False
                    ),  # 🆕 AI扩写润色状态
                    **enhancement_prompts,  # 🆕 包含AI扩写润色提示词信息
                },
            }
    ...
    def _build_enhanced_mapping_strategy(
        ...
        # 🎯 生成精确映射信息（包含时长信息，帮助AI选择10-15个关键截图）
        mapping_info = "\n### 时间段-帧图精确映射（预处理完成）：\n"
        mapping_info += (
            "💡 **智能配图建议**: 从以下映射中选择10-15个**最关键**的时间段配图\n"
        )
        mapping_info += "   - 优先选择**时长较长**(>5s)的时间段（内容更重要）\n"
        mapping_info += "   - 保证封面图 + 每个主要章节至少1张\n"
        mapping_info += "   - 避免过度配图，保持图文平衡\n\n"

        for match in matches[:20]:  # 显示前20个供选择
            start = match["subtitle"]["start"]
            end = match["subtitle"]["end"]
            duration = end - start  # 🆕 计算时长
            content = (
            ...
            mapping_info += f"- **{start:.1f}s-{end:.1f}s** (时长{duration:.1f}s, {duration_tag}): {content} → `{frame}`\n"

        if len(matches) > 20:
            mapping_info += (
                f"\n- ... 还有 {len(matches) - 20} 个时间段映射（可按需使用）\n"
            )
...
### 📋 关键帧使用规则：
1. **配图数量目标**: 全文使用**10-15张**关键帧（不是全部都用）
   - 根据时长和重要性智能选择
   - 优先使用标记为⭐⭐⭐和⭐⭐的时间段
   - 避免过度配图，保持阅读流畅性
2. **直接使用映射**: 每个选中的时间段已预先映射到精确的帧图
3. **封面图片**: 开头摘要部分必须包含封面图
4. **章节关键图**: 每个主要章节（## 标题）选择1-2张最相关的帧
5. **语义对齐**: 图片与文字内容在时间和语义上已精确对应
6. **关键位置强制配图**：
   - 📖 开头部分（摘要后）：1张封面图
   - 🔧 核心章节开头：每个重要章节1张图
   - 📊 关键概念处：概念解释配图
7. **图片分布要求**：确保主要章节都有合理配图（不要求全部）
...
3. **数学公式语法**（物理、数学、化学、生物等）：
   - **行内公式**：使用 `$公式$` 格式，例如：`$E=mc^2$`、`$F=ma$`
   - **独立公式**：使用 `$$公式$$` 格式，例如：`$$\\frac{{-b \\pm \\sqrt{{b^2-4ac}}}}{{{{2a}}}}$$`
   - **常用符号**：
     - 上标：`$x^2$`（x的平方）
     - 下标：`$H_2O$`（水分子）
     - 分数：`$\\frac{{{{1}}}}{{{{2}}}}$`（二分之一）
   ...
   - **化学式**：使用下标，如 `$CO_2$`、`$H_2SO_4$`
   - **物理单位**：紧跟数值，如 `速度为 $10 m/s$`
   - ⚠️ **重要**：涉及科学、数学、物理、化学、生物等内容时，**必须使用LaTeX公式**展示专业性
4. **禁用区域**：`# 总结` 和 `# 思考` 段落严禁插入图片
5. **格式禁忌**：
   - 禁止 `## # 摘要`、`## ## 步骤`等错误标题
   - ✅ 正确：`# 摘要`、`## 操作步骤`
6. **🚫 严禁AI助手语言**：
...
**科学公式示例**（如涉及数学、物理、化学等）：
- 行内公式：根据爱因斯坦质能方程 $E=mc^2$，我们可以计算...
- 独立公式：
$$F = ma$$
- 化学式：水分子 $H_2O$ 由两个氢原子和一个氧原子组成
- 复杂公式：一元二次方程的求根公式为 $$x = \\frac{{{{-b \\pm \\sqrt{{{{b^2-4ac}}}}}}}}{{{{2a}}}}$$
...
"""
    ...
    def _build_user_prompt_enhanced(
        self,
        transcript: str,
        enhanced_subtitles: List[Dict[str, Any]],
        keyframes_path,
        content_domain: str = "general",
    ) -> str:
        """构建增强版用户提示词 - 专门处理包含帧映射的转录"""

        # 从增强版转录中提取所有可用的帧
        available_frames = set()
        frame_mappings = []

        for subtitle in enhanced_subtitles:
            if "frame" in subtitle and subtitle["frame"]:
                frame_name = subtitle["frame"].split("/")[-1]  # 提取文件名
                available_frames.add(frame_name)
                frame_mappings.append(
                    {
                        "time_range": f"{subtitle['start']:.1f}-{subtitle['end']:.1f}s",
                        "text": subtitle["text"][:50]
                        + ("..." if len(subtitle["text"]) > 50 else ""),
                        "frame": frame_name,
                    }
                )

        available_frames_list = list(available_frames)

        # 构建精确映射指南
        mapping_guide = f"""
## 精确时间段-帧图映射（预处理完成）
已为您预先处理好每个时间段对应的精确帧图，可直接使用：

### 📊 映射统计
- **映射数量**: {len(frame_mappings)} 个精确匹配
- **可用图片**: {', '.join(available_frames_list)}
- **图片路径**: ![图片名](keyframes/图片名)

### 部分映射示例：
{chr(10).join([f"- **{m['time_range']}**: {m['text']} → `{m['frame']}`" for m in frame_mappings[:10]])}
{f"- ... 还有 {len(frame_mappings)-10} 个映射" if len(frame_mappings) > 10 else ""}

### ✅ 使用优势
- **零误解风险**: 无需AI推测帧文件名，直接使用预处理结果
- **精确对应**: 每个时间段已匹配到最佳帧图
- **语义对齐**: 图片与文字内容在时间和语义上精确对应
"""

        # 🔥 Few-shot 示例（使用实际的帧文件名）
        example = self._get_few_shot_example_enhanced(
            content_domain, keyframes_path, available_frames_list
        )

        return f"""{example}

【你的任务】
请基于以下转录内容和**预处理的帧图映射**，生成知识卡片：
...
**关键指令**：
- **直接使用映射**: 每个时间段已预先映射到精确的帧图，直接使用即可
- **封面图必须**: 开头摘要后必须有一张封面图片
- **章节配图要求**: 每个重要的 `## 章节` 都应该有对应的关键帧
- **确保图片分布**: 至少50%的主要章节都必须有配图
- **只能使用**: {', '.join(available_frames_list)}
- **图片格式**: `![xxx.jpg](keyframes/xxx.jpg)`
- **禁用区域**: `# 总结` 和 `# 思考` 段落禁止插入任何图片
- **关键位置强制配图**: 摘要后、核心章节开头、重要概念处
- 输出纯文本，不要额外解释
"""

    def _get_few_shot_example_enhanced(
        self, content_domain: str, keyframes_path: str, available_frames: list
    ) -> str:
        """为增强版映射生成Few-shot示例 - 使用实际可用的帧文件名"""

        # 从实际可用的帧中选择示例
        if len(available_frames) >= 4:
            example_frames = available_frames[:4]
        else:
            example_frames = available_frames + [
                "00_02.jpg",
                "00_10.jpg",
                "00_20.jpg",
                "00_30.jpg",
            ]
            example_frames = example_frames[:4]

        return f"""【输出示例 - 使用预处理映射】
# Python函数基础知识详解
...
## 实战演练
通过编写计算器函数，演示函数的实际应用。从简单的加法函数开始，逐步扩展到支持多种运算的通用计算器。
...
""",
    ...
    async def _generate_text_only_content(
        ...
        """生成纯文本内容（摘要模式）- 不依赖图片映射"""
        logger.info("开始生成纯文本内容（摘要模式）")

        # 构建摘要专用的系统提示词
        system_prompt = """你是一位专业的内容总结专家。请根据提供的视频转录文本生成高质量的摘要内容。

# 输出要求
1. 使用简洁明了的语言
2. 提炼关键信息和核心观点
3. 保持逻辑清晰的结构
4. 不要包含任何图片引用或markdown图片语法
5. 采用标准的markdown格式
...
请直接输出内容，不要添加任何解释性文字。"""
        ...
        # 构建用户提示词
        user_prompt = f"""请基于以下视频转录内容生成高质量的摘要：
...
请生成一份结构清晰、重点突出的摘要内容。"""
        ...
        # 简单的后处理（移除AI介绍性话语）
        content = self._post_process_summary_content(content)

        if stream_callback:
            await stream_callback(
                "ai_content_chunk", {"type": "summary", "content": content}
            )

        logger.info("✅ 纯文本内容生成完成")
        return content

    def _post_process_summary_content(self, content: str) -> str:
        """后处理摘要内容 - 移除AI介绍性话语"""
    ...
    async def _enhance_content_with_ai(
        ...
        **kwargs,
        ...
        """
        try:
            logger.info("开始AI扩写+润色处理")

            # 🎯 复用提示词构建方法，避免重复代码
            enhancement_system_prompt, enhancement_user_prompt = (
                self._build_enhancement_prompts(
                    original_content, transcript, content_domain
                )
            )

            # 1. 调用AI进行扩写润色（带重试机制）
            logger.info("🔄 正在调用AI进行扩写润色...")

            # AI扩写专用配置：更长超时时间和重试机制
            # 从配置文件读取超时设置
            enhancement_config = self._get_enhancement_config()

            enhanced_content = await self._call_ai_with_retry(
                enhancement_user_prompt,
                enhancement_system_prompt,
                max_tokens=3000,  # 扩写需要更多token
                temperature=0.7,  # 适中的创造性
                timeout=enhancement_config.get("timeout", 180),  # 🚀 扩写任务专用超时
                max_retries=enhancement_config.get("max_retries", 2),  # 可配置重试次数
            )

            # 2. 后处理增强内容
            enhanced_content = self._post_process_enhanced_content(enhanced_content)

            # 3. 质量检查：确保图片引用没有丢失
            original_images = re.findall(r"!\[.*?\]\(.*?\)", original_content)
            enhanced_images = re.findall(r"!\[.*?\]\(.*?\)", enhanced_content)

            if len(original_images) != len(enhanced_images):
                logger.warning(
                    f"⚠️ 图片引用数量不匹配: 原始={len(original_images)}, 增强后={len(enhanced_images)}"
                )
                # 如果图片引用丢失，采用保守策略：在原内容基础上轻微润色
                logger.info("🔄 采用保守润色策略...")
                enhanced_content = await self._conservative_enhancement(
                    original_content, content_domain
                )

            logger.info("✅ AI扩写+润色完成")

            # 6. 流式回调
            if stream_callback:
                await stream_callback(
                    "ai_content_chunk",
                    {"type": "content_card", "content": enhanced_content},
                )
            ...
            error_msg = str(e)
            logger.error(f"AI扩写+润色失败: {error_msg}")

            # 根据错误类型提供不同的降级策略
            if "timeout" in error_msg.lower() or "超时" in error_msg:
                logger.warning("⏰ AI请求超时，尝试保守润色策略...")
                try:
                    # 降级策略1：保守润色（更短的提示词，更少的token）
                    return await self._conservative_enhancement(
                        original_content, content_domain
                    )
                except Exception as conservative_error:
                    logger.error(f"保守润色也失败: {conservative_error}")
                    logger.warning("🔄 返回原始内容（未增强）")

            elif "connection" in error_msg.lower() or "网络" in error_msg:
                logger.warning("🌐 网络连接问题，返回原始内容")

            else:
                logger.warning("其他AI错误，返回原始内容")
            ...
            if stream_callback:
                await stream_callback(
                    "ai_content_chunk",
                    {
                        ...
                        "warning": f"AI扩写失败，已降级到原始内容: {error_msg}",
                    },
                )

            return original_content

    def _post_process_enhanced_content(self, content: str) -> str:
        """后处理增强内容"""
    ...
    async def _conservative_enhancement(
        self, original_content: str, content_domain: str
    ) -> str:
        """保守的润色策略 - 保持图片引用不变，短超时快速处理"""

        conservative_prompt = f"""请对以下{content_domain}领域的内容进行轻微润色，要求：

1. 保持所有图片引用完全不变
2. 仅优化语言表达，不增加新内容或建议
3. 严格基于转录内容，不引入外部知识/方法论
4. 改善句子结构和用词准确性
5. 保持原有长度和结构
...
请输出润色后的内容："""
        ...
        try:
            ...
            enhanced = await self._call_ai_with_retry(
                conservative_prompt,
                "",  # 无系统提示词，简化处理
                max_tokens=len(original_content) + 200,
                temperature=0.3,  # 低创造性，保守润色
                timeout=enhancement_config.get(
                    "conservative_timeout", 60
                ),  # 更短的超时时间
                max_retries=1,  # 只重试1次
            )

            return self._post_process_enhanced_content(enhanced)
        except Exception as e:
            logger.error(f"保守润色也失败: {e}")
            return original_content
    ...
    async def _call_ai_with_retry(
        ...
        **kwargs,
        ...
        for attempt in range(max_retries + 1):
            try:
                if attempt > 0:
                    logger.info(f"🔄 AI调用重试 {attempt}/{max_retries}")
                    # 指数退避：1秒、2秒、4秒
                ...
                last_error = f"AI请求超时 (超过{timeout}秒)"
                logger.warning(
                    f"⏰ {last_error} - 尝试 {attempt + 1}/{max_retries + 1}"
                )

            except Exception as e:
                last_error = str(e)
                logger.warning(
                    f"AI调用失败: {last_error} - 尝试 {attempt + 1}/{max_retries + 1}"
                )

                # 如果是网络相关错误，继续重试；否则直接抛出
                if (
                    "timeout" not in last_error.lower()
                    and "connection" not in last_error.lower()
                ):
                    raise e

        # 重试耗尽，抛出最后一次错误
        logger.error(f"🚫 AI调用重试耗尽，最后错误: {last_error}")
    ...
    def _get_enhancement_config(self) -> dict:
        ...
        try:
            ...
            if settings_path.exists():
                ...
                return settings.get("system", {}).get("ai_enhancement", {})
        except Exception as e:
            logger.warning(f"读取AI增强配置失败: {e}")

        # 返回默认配置
        return {"timeout": 180, "max_retries": 2, "conservative_timeout": 60}

    def _build_enhancement_prompts(
        self, original_content: str, transcript: str, content_domain: str
    ) -> tuple:
        """
        构建AI扩写润色的提示词
        ...
        """
        # 系统提示词
        enhancement_system_prompt = f"""你是一位资深内容编辑，负责对已有内容做“真实、保守”的润色优化。你必须严格遵循转录文本，不得引入外部知识或新增事实。

# 任务目标
对提供的内容卡片进行**轻度润色**，使其表达更清晰、结构更顺畅，但**不得新增转录中未出现的信息**。

# 扩写原则
1. **严格保持结构**：完全保持原文的章节结构、段落分布和图片位置
2. **仅做润色**：禁止扩写或新增信息，只允许改写与压缩
3. **图片位置固定**：所有图片引用必须保持在原有位置，不得移动或删除
4. **真实改写**：只允许改写已有表述，不得新增解释
5. **禁止外推**：不得加入案例、方法论、经验法则、常识性建议

# 润色要求  
1. **语言优化**：使用更精准、生动的词汇和表达
2. **逻辑强化**：确保内容逻辑清晰，各部分衔接自然
3. **格式美化**：优化markdown格式，提升视觉效果
4. **价值提升**：确保每个部分都有实际价值，避免冗余内容
5. **公式保护**：
   - **严格保持**所有LaTeX公式格式不变（`$公式$` 和 `$$公式$$`）
   - 如果原文有科学公式（物理、数学、化学、生物等），必须完整保留
   - 可以在公式周围增加解释说明，但公式本身不可改动
...
- **绝对禁止**改变章节结构
- **绝对禁止**修改LaTeX公式格式
- **绝对禁止**新增转录未出现的事实、建议、方法论、案例
- 只能在现有段落内容基础上进行扩写和润色
- 保持所有标题、列表、图片、公式的原有位置不变
...
请基于原内容和转录文本，生成润色后的高质量内容卡片。"""

        # 用户提示词
        enhancement_user_prompt = f"""请对以下内容卡片进行AI扩写和润色：
...
# 扩写润色要求
1. **严格保持结构**：保持所有图片引用位置不变
2. **仅做润色**：不扩写，不新增信息
3. **禁止重排**：不得改变章节、段落、图片的顺序
4. **真实改写**：只使用转录文本内的信息做更清晰的表达
5. **禁止外推**：不得添加经验法则、方法论、常识性建议
6. **语言优化**：优化语言表达和格式结构

请输出润色后的内容卡片："""

        return enhancement_system_prompt, enhancement_user_prompt
