    ...
    async def generate_content_card(
        ...
        **kwargs,
        ...
        try:
            ...
            # 处理force_domain：如果是"auto"或空，则使用智能分析结果
            if force_domain and force_domain != "auto":
                ...
                if force_domain == "auto":
                    logger.info(f"🤖 智能角色推荐模式: {content_domain}")
                else:
...
请输出润色后的内容："""
    ...
    def _build_enhancement_prompts(
        ...
        """
        # 系统提示词
        enhancement_system_prompt = f"""你是一位资深内容编辑，拥有多年的专业写作和内容优化经验。你精通{content_domain}领域知识，擅长将基础内容扩写为深度、实用的高质量文章，同时保持准确性和专业性。

# 任务目标
对提供的内容卡片进行智能扩写和润色，通过增加深度解释、实用案例和专业见解，将其提升为更具价值的学习资料。

# 扩写原则
1. **严格保持结构**：完全保持原文的章节结构、段落分布和图片位置
2. **按段落扩写**：针对每个自然段落进行扩写，不重新组织内容结构
3. **图片位置固定**：所有图片引用必须保持在原有位置，不得移动或删除
4. **深度扩展**：在现有要点基础上增加更详细的解释、举例和分析
5. **专业性提升**：结合专业知识，增加权威性内容
6. **实用性增强**：在不改变段落结构的前提下，添加实际应用建议
...
2. **按段落扩写**：只在现有段落内容基础上进行扩写
3. **禁止重排**：不得改变章节、段落、图片的顺序
4. **深度扩展**：在现有基础上深度扩展每个要点
5. **专业提升**：增加实用性建议和专业见解
6. **语言优化**：优化语言表达和格式结构
