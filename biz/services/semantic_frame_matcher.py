...
if __name__ == "__main__":
    ...
    async def test_semantic_matcher():
        ...
        # 测试数据
        test_frames = [
            ("frame_00_30.jpg", 30.0),
            ("frame_01_00.jpg", 60.0),
            ("frame_01_30.jpg", 90.0),
            ("frame_02_00.jpg", 120.0),
        ]

        test_transcript = """
        大家好，今天我来教大家制作宫保鸡丁。首先我们需要准备食材，
        鸡胸肉要切成丁状。然后准备调料，生抽、老抽、白糖等。
        接下来开始烹饪，先炸花生米，然后炒鸡丁。最后调味出锅，
        一道美味的宫保鸡丁就完成了。
        """
        ...
        # 执行语义匹配
        matches = await matcher.match_frames_to_content(
            test_frames, test_transcript, content_domain="cooking"
        )

        print(f"📊 匹配结果:")
        for match in matches:
            print(f"  帧: {match.frame_filename}")
            print(f"  匹配文本: {match.matched_text_segment[:50]}...")
            print(f"  语义分数: {match.semantic_score:.2f}")
            print(f"  匹配原因: {match.match_reason}")
            print("  ---")

        # 生成策略提示词
        strategy = matcher.generate_matching_strategy_prompt(matches, "/test/keyframes")
        print(f"\n🎨 策略提示词:\n{strategy[:300]}...")

    import asyncio

    asyncio.run(test_semantic_matcher())
