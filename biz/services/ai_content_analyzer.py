...
class AIContentAnalyzer:
    ...
    async def analyze_content(self, transcript: str, **kwargs) -> ContentAnalysisResult:
        ...
        try:
            ...
            # 确保领域分析结果不为None
            if domain_analysis is None or not isinstance(domain_analysis, dict):
                logger.warning("⚠领域分析结果无效，使用默认值")
                domain_analysis = {
            ...
            # 确保内容特征分析结果不为None
            if content_features is None or not isinstance(content_features, dict):
                logger.warning("⚠内容特征分析结果无效，使用默认值")
                content_features = {
                    "content_style": "通俗",
                    "target_audience": "一般用户",
                    "content_length": "中等",
...
请确保返回有效的JSON格式。"""
        ...
        try:
            ...
            logger.warning("⚠AI返回的JSON格式无效，使用默认分析")
            return {
                "main_theme": "未知主题",
                "key_topics": [],
                "content_summary": "内容分析失败",
...
{transcript[:2000]}"""
        ...
        try:
            ...
            if not response or not response.strip():
                logger.warning("⚠AI返回空响应，使用关键词分类")
            ...
            try:
                ...
                logger.warning(f"⚠AI返回的JSON格式无效: {json_error}")
...
- 节点简洁有力""",
    ...
    def _adjust_template_by_features(
        ...
        # 🔧 修复：安全访问 analysis_result 属性
        if analysis_result:
            target_audience = getattr(analysis_result, "target_audience", None)
            content_style = getattr(analysis_result, "content_style", None)
            key_topics = getattr(analysis_result, "key_topics", [])

            if target_audience and target_audience in audience_adjustments:
                adjusted_prompt += (
                    f"\n\n# 目标受众调整\n{audience_adjustments[target_audience]}"
                )

            if content_style and content_style in style_adjustments:
                adjusted_prompt += (
                    f"\n\n# 内容风格调整\n{style_adjustments[content_style]}"
                )
...
async def analyze_and_generate_prompt(
    ...
    if force_domain and force_domain != "auto":
        # 强制使用指定的角色，跳过智能分析
        logger.info(f"使用指定角色: {force_domain}")
