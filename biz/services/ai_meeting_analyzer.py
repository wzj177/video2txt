...
class AIMeetingAnalyzer:
    ...
    async def analyze_meeting_content(
        ...
        try:
            ...
            # 2. 章节速览分析
            chapters = await self._generate_chapter_overview(
            ...
            # 4. 问答回顾分析
            qa_review = await self._generate_qa_review(
                segments, full_transcript
            )
    ...
    async def _generate_chapter_overview(
        ...
        try:
            ...
            for segment in segments:
                ...
                # 如果超过章节时长，创建新章节
                if segment_start - chapter_start_time > chapter_duration and current_chapter:
    ...
    async def _generate_qa_review(
        ...
        try:
            ...
            for i, segment in enumerate(segments):
                ...
                # 检查是否是问题
                is_question = (
                    any(indicator in text for indicator in question_indicators) or
                    text.endswith("？") or
                    text.endswith("?")
                )
                ...
                if is_question:
                    ...
                    answer_text = " ".join([
                        ...
                        if seg.get("speaker") != current_question["speaker"]  # 不同说话人的回答
                    ])
    ...
    async def _extract_keywords(self, text: str) -> List[str]:
        ...
        # 停用词
        stop_words = {"的", "了", "是", "在", "我", "你", "他", "她", "和", "与", "或", "但", "然后", "这", "那", "这个", "那个"}
    ...
    def _determine_overall_tone(self, sentiment_distribution: Dict[str, float]) -> str:
        ...
        positive_score = sum([
            sentiment_distribution.get(emotion, 0) 
