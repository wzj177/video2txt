...
from biz.queue.task_manager import task, get_task_manager
...
def update_task_status_sync(task_id: str, update_data: Dict[str, Any]):
    """同步更新任务状态（用于非异步环境）"""
    try:
        # 使用asyncio运行异步任务更新
        loop = None
...
def process_video_file_task(
    ...
    try:
        ...
        # 步骤1: 文件预处理
        update_task_status_sync(
            task_id, {"progress": 15, "current_step": "文件预处理..."}
        )
        ...
        # 步骤2: 音频提取
        update_task_status_sync(
            task_id, {"progress": 30, "current_step": "音频提取..."}
        )
        ...
        # 步骤3: 语音识别
        update_task_status_sync(
            task_id, {"progress": 50, "current_step": "语音识别中..."}
        )
        ...
        # 步骤4: 生成摘要
        update_task_status_sync(
            task_id, {"progress": 75, "current_step": "生成摘要..."}
        )
        ...
        # 步骤5: 生成输出文件
        update_task_status_sync(
            task_id, {"progress": 90, "current_step": "生成输出文件..."}
        )
...
def process_video_url_task(task_id: str, url: str, config: Dict[str, Any]):
    """
    处理视频URL任务（SQLite队列版本）
    ...
    try:
        ...
        # 继续按文件处理流程
        return process_video_file_task(task_id, file_data, config)
